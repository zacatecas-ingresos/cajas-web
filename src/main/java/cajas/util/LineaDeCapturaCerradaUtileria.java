...
public class LineaDeCapturaCerradaUtileria {
	...
	static String condensarFechaVencimiento(Date _fecha) {
		...
		// validar el anio que se le resta al anio de la fecha a condensar ya
		// que segun la ultima actualizacion por parte de los bancos debia
		// restarse 2013.
	...
	public static String obtenerLineaCapturaOxxo(String folio, Date fechaVencimiento, String importe) {
		String lcc = "";
		String digitoInicial = "03";
		String digitoFinal = "4";

		DateFormat df = new SimpleDateFormat("yyyyMMdd");
		String fechaV = df.format(fechaVencimiento);

		

		int tamanioFolio = 10;
		int resFolio = tamanioFolio-folio.length();
		String complementoFolio="";
		if (resFolio > 0) {
			for (int i = 1; i <= resFolio; i++) {
               String digito = "0";
               complementoFolio = complementoFolio+digito;
			}
		}
		
		
		String importeNuevo = importe.replace(".", "");
		System.out.println("importeNuevo "+ importeNuevo);
		int tamanioImporte = 7;
		int resImporte = tamanioImporte - importeNuevo.length();
		String complementoImporte = "";
		if (resImporte > 0) {
			for (int i = 1; i <= resImporte; i++) {
               String digito = "0";
               complementoImporte = complementoImporte+digito;
			}
		}

		lcc = digitoInicial + complementoFolio+folio + fechaV + complementoImporte+importeNuevo + digitoFinal;
		System.out.println("LINEA DE CAPTURA GENERADA oxxo " + lcc);
		return lcc;
	}
